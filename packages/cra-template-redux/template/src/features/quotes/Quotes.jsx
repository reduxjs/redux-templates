...
export const Quotes = () => {
    ...
    if (isSuccess) {
        ...
        <select className={styles.select} value={numberOfQuotes} onChange={e => setNumberOfQuotes(Number(e.target.value))}>
          <option value="5">5</option>
          <option value="10">10</option>
          <option value="20">20</option>
          <option value="30">30</option>
        </select>
        {data.quotes.map(({ author, quote, id }) => (<blockquote key={id}>
            "{quote}"
            <footer>
              <cite>{author}</cite>
            </footer>
